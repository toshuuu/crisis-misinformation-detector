...
const { calcDistanceMeters, MAX_DISTANCE_METERS } = require('../utils/distance');
...
exports.verifyReport = async (req, res) => {
  try {
    ...
    // Reject if user is beyond 10 km
    if (distance > MAX_DISTANCE_METERS) {
      return res.status(403).json({ error: 'You must be within 10 km of the crisis to provide feedback' });
    }

    let distanceWeight = 0;
    if (distance < 2000) distanceWeight = 1.0;
    else if (distance < 5000) distanceWeight = 0.5;
    else distanceWeight = 0.2;
